...
create table if not exists nutricao_registros (
  id uuid primary key default gen_random_uuid(),
  data date not null default current_date,
  alimento_id uuid, nome text,
  qtd numeric, unidade text,
  cal numeric, prot numeric, carb numeric, gord numeric,
  criado_em timestamptz default now());
...
alter table nutricao_dia drop constraint if exists nutricao_dia_pkey;
create unique index if not exists nutricao_dia_user_data on nutricao_dia(user_id, data);
...
function criarTarefaParseada(p, ctx) {
  ...
  // senão no PRÓXIMO bloco a começar. ctx.bloco_id força (ex.: criada dentro de um bloco).
  const bloco_id = ctx.bloco_id !== undefined ? ctx.bloco_id : blocoParaTarefaNova({ projeto_id });
...
function taskItemHTML(t, o={}) {
  ...
  return '<div class="item task'+(t.concluida?' done':'')+'" data-tid="'+t.id+'" '+(o.drag?'draggable="true"':'')+'>'
    ...
    + (o.planAdd ? '<button class="btn small primary" data-act="plan-add" data-id="'+t.id+'" title="enviar ao plano do dia">➕ plano</button>' : '')
    + (o.planRemove ? '<button class="iconbtn" data-act="plan-remove" data-id="'+t.id+'" title="tirar do plano do dia">➖</button>' : '')
...
function capacidadeDia(data) {
  ...
  // se HÁ plano do dia, capacidade = soma das durações das tarefas NO plano (o que foi alocado);
  // sem plano, cai para todas as tarefas com vencimento no dia. Nunca bloqueia — só avisa.
  const plano = data === hoje() ? planoDia() : null;
  const base = plano ? planoTarefas() : T('tarefas').filter(t => !t.abandonada && t.vencimento === data);
  const min = sum(base.map(estMin));
...
function selecionarAteCaber(elegiveis, capMin) { // enche até caber; garante ao menos a 1ª (estouro permitido)
  const sel = []; let soma = 0;
  for (const t of elegiveis) { const d = estMin(t); if (!sel.length || soma + d <= capMin) { sel.push(t); soma += d; } }
  return sel;
}
/* início/fim EFETIVOS com esticamento (estouro) e empurrão dos blocos seguintes */
function blocosPlanoEsticados(data) {
  const bs = blocosPlano(data); let prevFim = 0; const out = [];
  for (const b of bs) {
    let ini = new Date(b.inicio).getTime();
    if (prevFim && ini < prevFim) ini = prevFim;          // empurra para depois do bloco anterior
    const fim = ini + blocoRealMin(b) * 60000;
    out.push({ b, iniMs: ini, fimMs: fim, esticou: blocoEsticou(b) });
    prevFim = fim;
  }
  return out;
}
function apagarBloco(id) { // remove o bloco e solta as tarefas (bloco_id = null) — sem erro
  for (const t of T('tarefas').filter(t => t.bloco_id === id)) dbPatch('tarefas', t.id, { bloco_id: null });
  dbDelete('blocos', id);
}
/* virada do dia: tarefas de blocos de dias anteriores soltam o bloco_id (viram atrasadas soltas).
   Os blocos em si ficam como histórico. */
function limparBlocosVencidos() {
  const blocoData = {}; for (const b of T('blocos')) blocoData[b.id] = dISO(new Date(b.inicio));
  for (const t of T('tarefas')) if (t.bloco_id && blocoData[t.bloco_id] && blocoData[t.bloco_id] < hoje()) dbPatch('tarefas', t.id, { bloco_id: null });
}
...
function blocoParaTarefaNova(t) {
  ...
  const futuros = cands.filter(b => new Date(b.inicio).getTime() > agora).sort((a,b)=>new Date(a.inicio)-new Date(b.inicio));
  return (futuros[0] || cands[0]).id;
}
...
/* ════════ TELA HOJE — 4 seções ════════
   1) Plano do dia (na ordem montada, dentro do limite de 8h)
   2) Hoje, fora do plano (venc. hoje, não entrou: projeto não escolhido ou passou de 8h)
   3) Atrasadas (venc. < hoje) — decidir destino ou puxar ao plano
   4) Restante da semana (amanhã até domingo) — puxar ao plano
   De qualquer seção 2/3/4 dá para "➕ plano" (enviar ao plano do dia, mesmo estourando 8h → só avisa). */
/* ordenação do dia: prioridade da tarefa (P1→P4) → menor duração → projeto (agrupa/desempata) → título */
const projNomeDe = t => { const p = t.projeto_id && byId('projetos', t.projeto_id); return p ? norm(p.nome) : '￿'; };
const ordDia = (a, b) => (a.prioridade||4) - (b.prioridade||4) || estMin(a) - estMin(b)
  || projNomeDe(a).localeCompare(projNomeDe(b)) || String(a.titulo||'').localeCompare(String(b.titulo||''));
/* tarefas do plano do dia, na ordem salva (só as existentes / não abandonadas) */
function planoTarefas() { const p = planoDia(); return p ? p.ordem.map(id => byId('tarefas', id)).filter(t => t && !t.abandonada) : []; }
const idsNoPlano = () => new Set((planoDia() || {}).ordem || []);

/* Seção 1 — Plano do dia. Cada item pode ser tirado do plano (➖). */
function hojePlanoHTML() {
  const janela = Math.max(1, Number(getCfg('janela_util', 8))) * 60;
  const p = planoDia();
  if (!p) return '<div class="card pad0"><div class="sec-head" style="padding:10px 14px 2px">🗓️ Plano do dia</div>'
    + '<div class="empty" style="padding:18px"><span class="em">🌤️</span>Dia ainda não planejado. Toque em <b>Planejar o dia</b> para montar a lista por prioridade dentro do limite de '+fmtMin(janela)+'.</div></div>';
  const ts = planoTarefas();
  const pend = ts.filter(t => !t.concluida), feitas = ts.filter(t => t.concluida);
  const totMin = sum(pend.map(estMin)), over = totMin > janela;
  return '<div class="card pad0"><div class="sec-head row" style="padding:10px 14px 2px">🗓️ Plano do dia <span class="badge acc">'+pend.length+'</span>'
    + '<span class="sp"></span><span class="tiny '+(over?'err':'muted')+'" style="padding-right:12px">⏳ '+fmtMin(totMin)+' / '+fmtMin(janela)+(over?' ⚠️':'')+'</span></div>'
    + '<div class="list" style="padding:0 10px 8px">'
    + (pend.map(t => taskItemHTML(t, {planRemove:true})).join('')
       || (ts.length ? '<div class="empty" style="padding:14px"><span class="em">🎉</span>Tudo do plano concluído!</div>'
                     : '<div class="empty" style="padding:14px"><span class="em">🌤️</span>Plano vazio. Envie tarefas das seções ao lado com ➕ plano, ou refaça o Planejar o dia.</div>'))
    + '</div>'
    + (feitas.length ? '<details class="help"><summary>✔️ Concluídas ('+feitas.length+')</summary><div class="list">'+feitas.map(t => taskItemHTML(t, {planRemove:true})).join('')+'</div></details>' : '')
    + '</div>';
}
/* Seção 2 — Hoje, fora do plano. */
function hojeForaPlanoHTML() {
  const noPlano = idsNoPlano();
  const fora = tarefasPendentes().filter(t => t.vencimento === hoje() && !noPlano.has(t.id)).sort(ordDia);
...
function gaugeHTML(pct, label, sub, cor) {
  pct = clamp(pct, 0, 1); const c = Math.PI * 26, off = c * (1 - pct);
  return '<div class="gauge"><svg viewBox="0 0 64 40" width="76" height="46">'
    + '<path d="M6 34 A26 26 0 0 1 58 34" fill="none" stroke="var(--border)" stroke-width="7" stroke-linecap="round"/>'
    + '<path d="M6 34 A26 26 0 0 1 58 34" fill="none" stroke="'+cor+'" stroke-width="7" stroke-linecap="round" stroke-dasharray="'+c+'" stroke-dashoffset="'+off+'"/>'
    + '<text x="32" y="31" text-anchor="middle" font-size="13" font-weight="800" fill="var(--txt)">'+Math.round(pct*100)+'%</text></svg>'
    + '<div class="gl">'+esc(label)+'</div><div class="gs muted tiny">'+esc(sub)+'</div></div>';
}
function blocoGaugesHTML() {
...
/* velocímetro único do dia — % concluído entre as tarefas DO PLANO (ou, sem plano, entre as de hoje) */
function velocimetroDiaHTML() {
  const plano = planoDia();
  const base = plano ? planoTarefas() : T('tarefas').filter(t => !t.abandonada && t.vencimento === hoje());
  if (!base.length) return '';
  const done = base.filter(t => t.concluida).length;
  const pct = done / base.length;
  const cor = pct >= 1 ? 'var(--ok)' : pct >= 0.5 ? 'var(--acc)' : 'var(--warn)';
  return '<div class="gauges">' + gaugeHTML(pct, plano ? 'Plano do dia' : 'Concluídas hoje', done + ' de ' + base.length, cor) + '</div>';
}
...
act('task-bloco-menu', el => {
  ...
  modal('<div class="bx-h"><div class="h2">Mover "'+esc(t.titulo)+'"</div><button class="iconbtn" data-act="m-close">✕</button></div>'
    ...
    + '<button data-act="task-tirar-dia" data-id="'+t.id+'"><span class="em">📤</span>Tirar do dia (vira solta)</button>'
    + '</div>');
});
act('task-mover-bloco', el => { dbPatch('tarefas', el.dataset.id, { bloco_id: el.dataset.b }); closeModal(); render(); });
act('task-tirar-dia', el => { dbPatch('tarefas', el.dataset.id, { bloco_id: null }); closeModal(); render(); toast('Tarefa tirada do dia.', {icone:'📤'}); });
...
function agendaDiaHTML(data) {
  ...
  let evs = '';
  // blocos de PLANEJAMENTO: posição/altura com esticamento e empurrão; clique = resumo
  for (const { b, iniMs, fimMs, esticou } of blocosPlanoEsticados(data)) {
    const di = new Date(iniMs);
    const top = (di.getHours() - H_INI) * PXH + di.getMinutes() * PXH / 60;
    const alt = Math.max(20, (fimMs - iniMs) / 36e5 * PXH - 2);
    const cor = corDoBloco(b); const ts = tarefasDoBloco(b);
    evs += '<div class="tl-ev" data-act="bloco-resumo" data-id="'+b.id+'" style="top:'+top+'px;height:'+alt+'px;background:'+cor+'1f;border-color:'+cor+'66;color:'+cor+'">'
...
reg('hoje', {
  ...
  render: () => {
    ...
    const pend = tarefasPendentes();
    const atrasadas = pend.filter(t => t.vencimento && t.vencimento < hoje());
    const deHoje = pend.filter(t => t.vencimento === hoje());
    ...
    let html = '<div class="h1">'+saud+'</div>'
      ...
      + deHoje.length+' tarefa'+(deHoje.length===1?'':'s')+' hoje'+(atrasadas.length?' <span class="err">(+'+atrasadas.length+' atrasadas)</span>':'')
      + ' · '+habPend+' hábito'+(habPend===1?'':'s')+' pendente'+(habPend===1?'':'s')+' · '+fmtMin(cap.min)+' no plano</p>';
...
/* ---- Planejar o dia — SELEÇÃO DE PROJETOS + CORTE DE 8h (sem blocos, sem SQL) ----
   ...
   prioridade da TAREFA (P1→P4) → menor duração (projeto só agrupa/desempata) e cortada
   no limite da janela útil (padrão 8h): o que passa fica "fora do plano" (mas continua
   sendo de hoje). Atrasadas e a semana ficam em seções próprias na tela Hoje, de onde
   você pode enviar ao plano quando quiser. O plano é salvo em `configuracoes` (chave
   plano_dia): { data, projetos: [chaves selecionadas], ordem: [ids no plano] }. */
...
function _planCorte(tarefas, janela) {
  let soma = 0, cut = tarefas.length;
  for (let i = 0; i < tarefas.length; i++) { const d = estMin(tarefas[i]); if (i === 0 || soma + d <= janela) soma += d; else { cut = i; break; } }
  return { cut, soma };
}
function planDraw() {
  const box = $('#plan-box'); if (!box) return;
  const P = window._plan, janela = janelaMin();
  const chips = P.chaves.map(k => {
    const proj = k === '__none__' ? null : byId('projetos', k); const on = P.sel.has(k);
    return '<button class="chip mini'+(on?' sel':'')+'" data-act="plan-proj" data-k="'+esc(k)+'">'+(on?'✓ ':'')
      + (proj ? projLabelHTML(proj) : '📥 Sem projeto')+' <span class="muted tiny">'+P.grupos[k].length+'</span></button>';
  }).join(' ');
  const tarefas = _planOrdenado();
  const { cut, soma } = _planCorte(tarefas, janela);
  const linha = (t, dentro) => { const proj = byId('projetos', t.projeto_id);
    return '<div class="item" style="cursor:default;opacity:'+(dentro?1:.5)+'">'
      + '<span class="check p'+(t.prioridade||4)+'" style="pointer-events:none"></span>'
      + '<div class="grow"><div class="ttl">'+esc(t.titulo)+'</div><div class="sub">'
      + (proj ? projLabelHTML(proj)+' · ' : '') + 'P'+(t.prioridade||4)+' · ⏳ '+fmtMin(estMin(t))+'</div></div></div>'; };
  let lista = '';
  tarefas.forEach((t, i) => {
    if (i === cut) lista += '<div class="tiny center" style="padding:7px 6px;color:var(--warn);border-top:1px dashed var(--border)">— limite de '+fmtMin(janela)+' atingido · o que segue fica fora do plano —</div>';
    lista += linha(t, i < cut);
  });
  ...
  box.innerHTML = '<div class="bx-h"><div class="h2">☀️ '+(temPasso1?'2/2 · ':'')+'Planejar o dia</div><button class="iconbtn" data-act="m-close">✕</button></div>'
    ...
    + (lista || '<div class="empty" style="padding:16px"><span class="em">🌤️</span>Nenhuma tarefa nos projetos escolhidos.</div>')+'</div></div>'
    + '<div class="row" style="margin:8px 0"><span class="small '+(over?'err':'ok')+'">No plano: '+fmtMin(soma)+' / '+fmtMin(janela)+(over?' — acima do limite ⚠️':'')+'</span>'
    + '<span class="sp"></span>'+(foraN?'<span class="small muted">'+foraN+' fora do plano</span>':'')+'</div>'
    + '<div class="bx-foot"><button class="btn ghost" data-act="m-close">Cancelar</button><button class="btn primary big" data-act="plan-concluir"'+(cut?'':' disabled')+'>✓ Montar lista do dia</button></div>';
}
...
act('plan-add', el => {
  const t = byId('tarefas', el.dataset.id); if (!t) return;
  if (t.vencimento !== hoje()) dbPatch('tarefas', t.id, { vencimento: hoje() });
  const p = planoDia();
  const projs = p && Array.isArray(p.projetos) ? p.projetos.slice() : [];
  const pk = t.projeto_id || '__none__'; if (!projs.includes(pk)) projs.push(pk);
  const ordem = p ? p.ordem.slice() : [];
  if (!ordem.includes(t.id)) ordem.push(t.id);
  salvarPlanoDia(projs, ordem);
  dbUpsert('dias', { data: hoje(), ...(diaRow()||{}), planejado: true });
  const cap = capacidadeDia();
  if (cap.pct > 1) toast('“'+esc(t.titulo)+'” no plano — dia acima de '+fmtMin(cap.janela)+' ('+fmtMin(cap.min)+') ⚠️', {icone:'⚠️', ms:3800});
  else toast('“'+esc(t.titulo)+'” no plano ✓', {icone:'📌'});
  render();
});
/* tirar do plano — a tarefa continua de hoje (some para "Hoje, fora do plano") */
act('plan-remove', el => {
  const p = planoDia(); if (!p) { render(); return; }
  salvarPlanoDia(p.projetos, p.ordem.filter(id => id !== el.dataset.id));
  render();
});
/* ---- Planejar (ritual antigo de 3 passos — desativado, mantido p/ referência) ---- */
act('ritual-planejar', () => planejarDia());
function _ritualPlanejarAntigo() {
...
/* ---- Registro diário + barras de progresso (núcleo) ---- */
const round1 = x => Math.round((Number(x) || 0) * 10) / 10;
const registrosDoDia = data => ordenar(T('nutricao_registros').filter(r => r.data === (data || hoje())), r => r.criado_em || '');
const calcMacros = (a, qtd) => { const f = a.base === 'unidade' ? qtd : qtd / 100; return { cal:(a.cal||0)*f, prot:(a.prot||0)*f, carb:(a.carb||0)*f, gord:(a.gord||0)*f }; };
function somaMacros(regs) { return regs.reduce((s, r) => ({ cal:s.cal+(+r.cal||0), prot:s.prot+(+r.prot||0), carb:s.carb+(+r.carb||0), gord:s.gord+(+r.gord||0) }), {cal:0,prot:0,carb:0,gord:0}); }
function barraNutri(label, consumido, meta, un) {
  consumido = Math.round(consumido); meta = Number(meta) || 0;
  const pct = meta > 0 ? consumido / meta : 0;
  const cls = !meta ? 'ok' : pct > 1 ? 'err' : pct > 0.9 ? 'warn' : 'ok';
  const dir = meta ? (consumido <= meta ? ' · faltam ' + (meta - consumido) + un : ' · +' + (consumido - meta) + un + ' acima') : '';
  return '<div style="margin-bottom:9px"><div class="row" style="margin-bottom:3px"><span class="small bold">' + label + '</span><span class="sp"></span>'
    + '<span class="small ' + cls + '">' + consumido + un + (meta ? ' / ' + Math.round(meta) + un : ' (sem meta)') + dir + '</span></div>'
    + '<div class="bar thick"><i class="' + cls + '" style="width:' + clamp(pct * 100, 2, 100) + '%"></i></div></div>';
}
function nutriDiaHTML() {
  const p = nutricaoPerfil() || {};
  const regs = registrosDoDia(hoje());
  const t = somaMacros(regs);
  let html = '<div class="card"><div class="card-h"><div class="h2">📊 Hoje</div><button class="btn small primary" data-act="nutri-add">+ Registrar alimento</button></div>'
    + barraNutri('Calorias', t.cal, p.meta_cal, ' kcal')
    + barraNutri('Proteína', t.prot, p.meta_prot, ' g')
    + barraNutri('Carboidrato', t.carb, p.meta_carb, ' g')
    + barraNutri('Gordura', t.gord, p.meta_gord, ' g')
    + (!p.meta_cal && !p.meta_prot ? '<div class="tiny muted" style="margin-top:4px">Defina as metas abaixo para ver o saldo restante.</div>' : '')
    + '</div>';
  html += '<div class="card pad0"><div class="sec-head" style="padding:12px 14px 4px">🍽️ Registros de hoje ('+regs.length+')</div><div class="list" style="padding:0 10px 8px">'
    + (regs.length ? regs.map(r => '<div class="item"><div class="grow"><div class="ttl">'+esc(r.nome||'avulso')
        + (r.qtd ? ' <span class="muted small">'+fmtNum(r.qtd)+(r.unidade==='un'?' un':' g')+'</span>' : '')+'</div>'
        + '<div class="sub">⚡'+fmtNum(r.cal||0)+' kcal · P'+fmtNum(r.prot||0)+' C'+fmtNum(r.carb||0)+' G'+fmtNum(r.gord||0)+'</div></div>'
        + '<button class="btn small danger" data-act="nutri-del" data-id="'+r.id+'">✕</button></div>').join('')
      : '<div class="empty" style="padding:14px"><span class="em">🍽️</span>Nada registrado hoje. Toque em "+ Registrar alimento".</div>')
    + '</div></div>';
  return html;
}
function registrarNutri(r) { dbUpsert('nutricao_registros', { data: hoje(), alimento_id:null, qtd:null, unidade:null, carb:null, gord:null, ...r }); }
...
function nutriAddListaDraw(q) {
  ...
  lista.innerHTML = as.length
    ? as.map(a => '<div class="item" data-act="nutri-pick" data-id="'+a.id+'"><span>'+(a.favorito?'⭐':'🍽️')+'</span><div class="grow"><div class="ttl">'+esc(a.nome)+'</div><div class="sub">'+alimBaseLabel(a.base)+' · ⚡'+fmtNum(a.cal||0)+' P'+fmtNum(a.prot||0)+'</div></div></div>').join('')
    : '<div class="tiny muted" style="padding:8px">Nenhum alimento. Cadastre no Banco de alimentos abaixo.</div>';
}
act('nutri-busca', el => nutriAddListaDraw(el.value));
act('nutri-pick', el => { const a = byId('alimentos', el.dataset.id); if (!a) return;
  const un = a.base === 'unidade' ? 'unidade(s)' : 'gramas';
  modal('<div class="bx-h"><div class="h2">'+esc(a.nome)+'</div><button class="iconbtn" data-act="m-close">✕</button></div>'
    + '<div class="field"><label>Quantidade ('+un+')</label><input class="input" id="nutri-qtd" type="number" inputmode="decimal" value="'+(a.base==='unidade'?'1':'100')+'"></div>'
    + '<div id="nutri-prev" class="small muted" style="margin-bottom:6px"></div>'
    + '<div class="bx-foot"><button class="btn ghost" data-act="m-close">Cancelar</button><button class="btn primary" data-act="nutri-add-confirm" data-id="'+a.id+'">Adicionar</button></div>',
    { onMount: ov => { const i = ov.querySelector('#nutri-qtd');
        const upd = () => { const m = calcMacros(a, Number(i.value)||0); ov.querySelector('#nutri-prev').textContent = '⚡'+Math.round(m.cal)+' kcal · P'+round1(m.prot)+' C'+round1(m.carb)+' G'+round1(m.gord); };
        i.addEventListener('input', upd); upd(); setTimeout(() => { i.focus(); i.select(); }, 60); } });
});
act('nutri-add-confirm', el => {
...
act('nutri-min', () => { closeModal(true); editModal({ titulo:'Modo mínimo', salvar:'Adicionar',
  fields:[ {k:'nome', l:'Descrição (opcional)', ph:'ex.: refeição fora'}, {k:'cal', t:'num', l:'Calorias (kcal)', meia:1, foco:1}, {k:'prot', t:'num', l:'Proteína (g)', meia:1} ],
  onSave: v => { registrarNutri({ nome:(v.nome||'avulso'), cal:Math.round(v.cal||0), prot:round1(v.prot||0) }); render(); toast('Adicionado ✓', {icone:'⚡'}); } }); });

/* ---- Checklist de aderência + streak (Etapa 4) ---- */
const nutricaoDiaRow = data => byId('nutricao_dia', data || hoje());
const setNutriDia = patch => { const d = nutricaoDiaRow(hoje()) || { data: hoje() }; dbUpsert('nutricao_dia', { ...d, ...patch }); };
const nutriChecklistCompleto = d => !!(d && d.chk_prot && d.chk_agua && d.chk_treino && d.chk_cal);
function nutriStreak() {
  let n = 0, dia = hoje();
  if (!nutriChecklistCompleto(nutricaoDiaRow(hoje()))) dia = addDias(hoje(), -1); // tolera hoje ainda incompleto
  for (let i = 0; i < 400 && nutriChecklistCompleto(nutricaoDiaRow(dia)); i++) { n++; dia = addDias(dia, -1); }
  return n;
}
function nutriRecorde() {
  const dias = T('nutricao_dia').filter(nutriChecklistCompleto).map(d => d.data).sort();
  let best = 0, run = 0, prev = null;
  for (const d of dias) { run = (prev && diffDias(prev, d) === 1) ? run + 1 : 1; best = Math.max(best, run); prev = d; }
  return best;
}
function nutriChecklistHTML() {
  const d = nutricaoDiaRow(hoje()) || {};
  const item = (k, label) => '<span class="chip'+(d[k]?' sel':'')+'" data-act="nutri-check" data-k="'+k+'">'+(d[k]?'✅':'⬜')+' '+label+'</span>';
  const streak = nutriStreak(), rec = nutriRecorde();
  return '<div class="card"><div class="card-h"><div class="h2">✅ Checklist do dia</div><span class="badge ok">🔥 '+streak+' dia'+(streak===1?'':'s')+'</span></div>'
    + '<div class="row wrap" style="gap:8px">'+item('chk_prot','Proteína')+item('chk_agua','Água')+item('chk_treino','Treino')+item('chk_cal','Calorias')+'</div>'
...
function fluxoTabHTML(ym) {
  ...
  // saídas e entradas por categoria (pizza + linhas), com drill-down por subcategoria.
  // Montados a partir de `todos`, e não de v_movimentacoes_categoria_mes: a view agrega e
  // perde `descricao`, sem a qual não dá para separar os pagadores do salário. É a mesma
  // tabela e o mesmo filtro de ano_mes, então os totais continuam batendo com os KPIs do
  // topo. Inclui operacional + financiamento (empréstimos), já que ambos somam em
  // entradas/saídas; investimento fica de fora (patrimônio, não consumo).
...
/* Drill-down de uma categoria: total consolidado por subcategoria (sem listar os
   lançamentos individuais — detalhe demais para esta tela). */
function movCatDetalheHTML(itens) {
  const porSub = {};
  itens.forEach(l => {
    const s = (l.categoria_geral || '—') + ' › ' + (l.categoria_especifica || '—');
    porSub[s] = (porSub[s] || 0) + Number(l.valor);
  });
  return Object.entries(porSub)
    .sort((a, b) => b[1] - a[1])
    .map(([sub, total]) => '<span>'+esc(sub)+' — '+fmtBRL(total)+'</span>')
    .join('');
}
